...
class ActionSaveContactInfo(Action):

    def name(self) -> Text:
        return "action_save_contact_info"

    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        
        contact_info = next(tracker.get_latest_entity_values("contact_info"), None)
        if contact_info:
            dispatcher.utter_message(text=f"Got it! Your contact info is {contact_info}.")
